...
class BuildPage extends StatelessWidget {
  BuildPage({Key? key, required this.app_dir}) : super(key: key);
  ...
  translate() async {
    Shell shell = Shell(workingDirectory: app_dir);
...
echo "\033[0;32m✔\033[0m 搬移語系檔案"
  ...
  makeApkKey() async {
    Shell shell = Shell(workingDirectory: app_dir + "/android");
    await shell.run('''
...
echo "\033[0;32m✔\033[0m 生成IPA"
      ...
      //搬移檔案
      await shell.run('''
mkdir -p $app_dir/publish/outputs
mkdir -p $app_dir/publish/outputs/uat
mkdir -p $app_dir/publish/outputs/uat/$i
    ''');
      shell = shell.pushd("publish/outputs/uat/$i");
      await shell.run('''
mv $app_dir/build/app/outputs/flutter-apk/app-v1-release.apk ./app-release.apk
mv $app_dir/build/app/outputs/flutter-apk/app-v2-release.apk ./app-release-v2.apk
mv $app_dir/build/ios/iphoneos/Payload.ipa ./Payload.ipa
